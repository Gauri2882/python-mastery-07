...
def plot_graph():
    ...
    if data_choice == "1":
        x = list(map(float, input("Enter X values separated by spaces (eg: 1 2 3): ").split()))
        y = list(map(float, input("Enter Y values separated by spaces (eg: 1 2 3): ").split()))
    elif data_choice == "2":
    ...
    if save_choice == "yes":
        file_name = input("Enter the file name with extension (eg.: graph.png): ")
